...
export default function BetaInvitesPage() {
  ...
  useEffect(() => {
    if (session?.user) {
      setIsBetaTester((session.user as any).isBetaTester || false)
  ...
  useEffect(() => {
    ...
    const userIsBetaTester = (session.user as any)?.isBetaTester
    const userIsAdmin = (session.user as any)?.isAdmin

    if (!userIsBetaTester && !userIsAdmin) {
      router.push('/grove')
    }
      ...
      <main className="container mx-auto px-4 py-12 max-w-2xl">
        <div className="mb-8">
          ...
          <p className="text-text-muted">
            Share Firefly Grove with friends and family! Send invites via email or text message.
          </p>
        </div>
        ...
        <div className="mt-8 space-y-4">
          <div className="p-4 bg-bg-elevated rounded-lg border border-border-subtle">
            ...
            <ul className="text-sm text-text-muted space-y-1 list-disc list-inside">
              ...
              <li>Beta perks: Family Grove plan (10 trees) free during beta</li>
            </ul>
          </div>
          ...
          <div className="p-4 bg-firefly-dim/10 rounded-lg border border-firefly-dim/30">
            <h3 className="text-sm font-medium text-firefly-glow mb-2">✨ Help Firefly Grove Grow</h3>
            <p className="text-sm text-text-muted">
              As a beta tester, you're helping build something special. Every friend you invite helps us create a better platform for preserving family memories. Your early support means everything.
            </p>
          </div>
        </div>
      </main>
