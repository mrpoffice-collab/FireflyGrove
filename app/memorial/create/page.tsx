...
export default function CreateMemorialPage() {
  ...
  // Trustee contact info (no account required)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Redirect to the memorial page with name, dates, and branch ID
        router.push(`/memorial/created?name=${encodeURIComponent(name)}&birthDate=${encodeURIComponent(birthDate)}&deathDate=${encodeURIComponent(deathDate)}&branchId=${data.branch.id}`)
      } else {
          ...
          <div className="bg-bg-dark border border-border-subtle rounded-lg p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-text-soft">Your Contact Info</h3>
                ...
                <div>
                  ...
                  <p className="text-text-muted text-xs mt-1">
                    We'll send you a link to manage this memorial
                  </p>
                </div>
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-text-soft">Memorial Details</h3>

                {/* Name of person being memorialized */}
                <div>
                  <label className="block text-sm text-text-soft mb-2">
                    Name of Loved One <span className="text-error-text">*</span>
                  </label>
                  <input
                    type="text"
                    value={name}
