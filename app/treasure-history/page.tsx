...
export default function TreasureHistoryPage() {
  ...
  const startEdit = (entry: TreasureEntry) => {
  ...
  const saveEdit = async (entryId: string) => {
    if (!editText.trim()) {
      alert('Treasure text cannot be empty')
      return
    }

    try {
      const res = await fetch('/api/treasure/edit', {
        method: 'PATCH',
      ...
      if (res.ok) {
        ...
        setEntries(entries.map(entry =>
          entry.id === entryId ? { ...entry, text: editText.trim() } : entry
        ))
        setEditingId(null)
        setEditText('')
      } else {
                        ...
                        >
                          ...
                          <div className="absolute top-3 right-3 flex gap-2">
                            ...
                            <button
                              ...
                              disabled={editingId === entry.id || deletingId === entry.id}
                              className="text-text-muted hover:text-firefly-glow transition-soft disabled:opacity-50"
                              title="Edit this treasure"
                            >
                            ...
                            <button
                              ...
                              disabled={deletingId === entry.id || editingId === entry.id}
                            ...
                            editingId === entry.id ? (
                              <div className="mb-3">
                                <textarea
                                  ...
                                  className="w-full min-h-[100px] bg-bg-elevated border border-firefly-dim/30 rounded-lg p-3 text-text-soft focus:outline-none focus:ring-2 focus:ring-firefly-glow"
                                  autoFocus
                                />
                                <div className="flex gap-2 mt-2">
                                  ...
                                  <button
                                    onClick={cancelEdit}
                                    className="px-4 py-2 bg-bg-elevated hover:bg-border-subtle text-text-soft rounded-lg text-sm font-medium transition-soft"
                                  >
                                    Cancel
                                  </button>
                                </div>
