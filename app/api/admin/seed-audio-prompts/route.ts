...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingCount = await prisma.audioPrompt.count()
    if (existingCount > 0) {
      return NextResponse.json({
        message: `Already seeded. ${existingCount} prompts exist.`,
        count: existingCount,
      })
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully seeded ${audioPrompts.length} audio prompts`,
      count: audioPrompts.length,
    })
  } catch (error) {
    console.error('Error seeding audio prompts:', error)
    return NextResponse.json(
